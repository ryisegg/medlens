...
export function DrugDetailPage({ drug }: DrugDetailPageProps) {
  ...
  const translatableSections = useMemo(() => [
    ...
    { id: "mechanism", text: drug.mechanism },
    { id: "uses", text: drug.uses.join("\n") },
    { id: "commonDoses", text: drug.commonDoses.join("\n") },
    { id: "sideEffects", text: drug.sideEffects.map((item) => item.effect).join("\n") },
    { id: "contraindications", text: drug.contraindications.join("\n") },
    { id: "interactions", text: drug.interactions.map((item) => `${item.drugName}: ${item.description}`).join("\n") },
    { id: "pregnancy", text: drug.pregnancyNote },
    { id: "breastfeeding", text: drug.breastfeedingNote },
    { id: "doctor", text: drug.whenToCallDoctor.join("\n") },
    { id: "emergency", text: drug.emergencySigns?.join("\n") ?? "" },
  ...
  const translatedLines = (id: string, fallback: string[]) => translatedTextById[id]?.split(/\n+/).filter(Boolean) ?? fallback;
  const translatedSideEffects = translatedTextById.sideEffects?.split(/\n+/).filter(Boolean);
  const translatedInteractions = translatedTextById.interactions?.split(/\n+/).filter(Boolean);
  ...
  async function handleTranslate() {
    ...
    try {
      const result = await translateSections({
        targetLanguage: "zh",
        context: `${drug.name} curated medication profile`,
  ...
  return (
    <div className="space-y-3 px-4 py-4">
      <Card>
        ...
        <p className="mt-3 text-sm text-slate-700 leading-relaxed dark:text-[#8e8e93]">{translated("description", drug.description)}</p>
        ...
        {isZh && translatableSections.length > 0 && (
          <div className="mt-4">
            ...
            {translationError && (
              <p className="mt-2 text-xs text-amber-700 dark:text-amber-300">翻译暂时不可用：{translationError}</p>
            )}
          </div>
        )}
      </Card>

      <WarningBanner level="caution" title={t.drug.disclaimer} message={translationDisclaimer ?? t.drug.disclaimerDesc} />

      <Card>
        <SectionTitle>{language === "zh" ? `${t.drug.uses} / Uses` : t.drug.uses}</SectionTitle>
        <ul className="space-y-2">
          {translatedLines("uses", drug.uses).map((use, i) => (
      ...
      <Card>
        ...
        <p className="text-sm text-slate-700 leading-relaxed dark:text-[#8e8e93]">{translated("mechanism", drug.mechanism)}</p>
      </Card>
      ...
      <Card>
        ...
        <ul className="space-y-1">
          {translatedLines("commonDoses", drug.commonDoses).map((dose, i) => (
                ...
                <div className="flex flex-wrap gap-1.5">
                  {effects.map((s, i) => (
                    <span key={i} className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${SEVERITY_STYLE[s.severity]}`}>
                      {translatedSideEffects?.[drug.sideEffects.findIndex((effect) => effect.effect === s.effect)] ?? s.effect}
                    </span>
                  ))}
                </div>
              </div>
      ...
      <Card>
        ...
        <ul className="space-y-2">
          {translatedLines("contraindications", drug.contraindications).map((c, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-slate-700 dark:text-[#8e8e93]">
          ...
          <div className="space-y-2">
            {drug.interactions.map((inter, i) => (
              <InteractionWarning
                ...
                description={translatedInteractions?.[i]?.replace(`${inter.drugName}:`, "").trim() ?? inter.description}
              />
            ))}
          </div>
        </Card>
      ...
      <Card>
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-700 dark:text-[#8e8e93]">
            <span className="font-medium text-slate-900 dark:text-white">{t.drug.pregnancyNote}:</span> {translated("pregnancy", drug.pregnancyNote)}
          </p>
          <p className="text-sm text-slate-700 dark:text-[#8e8e93]">
            <span className="font-medium text-slate-900 dark:text-white">{t.drug.breastfeedingNote}:</span> {translated("breastfeeding", drug.breastfeedingNote)}
          </p>
        </div>
      </Card>
      ...
      <div className="rounded-2xl border-2 border-red-200 bg-red-50 px-5 py-4 dark:border-red-800 dark:bg-red-950/40">
        ...
        <ul className="space-y-2">
          {translatedLines("doctor", drug.whenToCallDoctor).map((item, i) => (
      ...
      {drug.emergencySigns && drug.emergencySigns.length > 0 && (
        <div className="rounded-2xl border-2 border-red-500 bg-red-50 px-5 py-4 dark:border-red-600 dark:bg-red-950/60">
          ...
          <ul className="space-y-2">
            {translatedLines("emergency", drug.emergencySigns).map((sign, i) => (
